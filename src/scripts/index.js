(function() {
    ...
    var night = {
        type: 'positive',
        icon: 'mapa_actividad_estrella.png'
    }
    ...
    var negativeWords = [
        ...
        "infeliz", "mal", "lo siento", "molest", "aversión", "ansios", "avergonzad", "irritable", "mierda", "chivo", "envidia",
    ...
    d3.json('json/spain-province.json', function(error, topology) {
        g.selectAll('path')
            ...
            .attr('class', function(d) {
                return 'states ' + d.properties.NAME_2;
            })
            .attr('d', path)
    ...
    /*function getData() {
        pubnub.history({
            ...
            response.messages.forEach(processData);
            getStreamData();
        });
    }*/
    ...
    function displayData(data, emotion) {
        ...
        getUserInfo(data, function(user) {
            ...
            if (document.querySelector('.' + user.state)) {
                tally[user.state] = (tally[user.state] || {
                    positive: 0,
                    negative: 0
                });
    ...
    function processData(data) {
        ...
        if (nightWords.some(function(v) {
            ...
            displayData(data, night);
        } else if (positiveWords.some(function(v) {
                return data.text.toLowerCase().indexOf(v) !== -1;
            })) {
